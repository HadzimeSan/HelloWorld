...
adaptHeight_fn = function () {
  console.log(
    this.clientHeight,
    this.selectedCell,
    this.selectedCell.clientHeight
  );
...
document.addEventListener("DOMContentLoaded", function () {
  if (window.location.href.includes("ergonliplap-mixing-and-mastering-pro")) {
    // Create script tag for gtag.js
    const gtagScript = document.createElement("script");
    gtagScript.async = true;
    gtagScript.src =
      "https://www.googletagmanager.com/gtag/js?id=AW-16697626457";
    document.head.appendChild(gtagScript);

    // Create script tag for GTM configuration
    const configScript = document.createElement("script");
    configScript.innerHTML = `
    window.dataLayer = window.dataLayer || [];
    function gtag(){dataLayer.push(arguments);}
    gtag('js', new Date());
gtag('config', 'AW-16697626457');
  `;
    document.head.appendChild(configScript);

    // Create script tag for page_view event
    const eventScript = document.createElement("script");
    eventScript.innerHTML = `
    gtag('event', 'page_view', {
'send_to': 'AW-16697626457',
 ...
 let popup = document.querySelector("#popup-pro-1");
		let popup2 = document.querySelector("#popup-pro-2");
		let popup3 = document.querySelector("#popup-pro-3");
		let popups = [popup, popup2, popup3];

		popups.forEach((popup) => {
			//check if inputs are filled
			let inputs = popup.querySelectorAll("input");
			let submit = popup.querySelector(".modal-submit-btn");
			submit.addEventListener("click", () => {
				let skipbtn = popup.querySelector(".skip-button");
				let filled = true;
